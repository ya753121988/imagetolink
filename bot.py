...
app = Flask(__name__)
app.secret_key = "ultimate_private_cloud_engine_v10"

# --- MongoDB Connection ---
MONGO_URI = "mongodb+srv://roxiw19528:roxiw19528@cluster0.vl508y4.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
client = MongoClient(MONGO_URI)
db = client['poster_pro_v10']
fs = gridfs.GridFS(db) # নিজস্ব ক্লাউড স্টোরেজ
...
<style>
    ...
    body { background: var(--dark); color: #f1f5f9; font-family: 'Plus Jakarta Sans', sans-serif; margin: 0; overflow-x: hidden; }
    .glass { background: var(--glass); backdrop-filter: blur(12px); border: 1px solid rgba(255,255,255,0.1); border-radius: 20px; }
    .btn-grad { background: linear-gradient(135deg, var(--primary), var(--accent)); border: none; color: white; border-radius: 12px; font-weight: 700; transition: 0.3s; }
    .btn-grad:hover { transform: translateY(-3px); box-shadow: 0 10px 20px rgba(99, 102, 241, 0.4); color: white; }
    .nav-bar { background: rgba(15, 23, 42, 0.8); backdrop-filter: blur(10px); border-bottom: 1px solid rgba(255,255,255,0.1); position: sticky; top: 0; z-index: 1000; }
...
def home():
    ...
    <head>
        ...
        <title>Poster Cloud - Private Hosting</title>
        <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
        {{ style | safe }}{{ s.ad_social | safe }}{{ s.ad_popunder | safe }}
    </head>
    <body>
        <div class="nav-bar p-3"><div class="container d-flex justify-content-between align-items-center"><span class="fw-bold">🚀 POSTER CLOUD</span><a href="/login" class="btn btn-outline-light btn-sm">Admin</a></div></div>
        <div class="container py-5">
            <div class="text-center mb-5">
                <h1 class="fw-bold display-5">Upload & <span class="text-primary">Host Private</span></h1>
                <p class="text-secondary">Get 16+ high-speed links directly from our MongoDB server.</p>
            </div>
            <div class="row justify-content-center mb-5">
                <div class="col-md-7">
                    <div class="glass p-5 text-center">
                        <form action="/api/upload" method="POST" enctype="multipart/form-data" id="upForm">
                            <label for="file" style="cursor:pointer;" class="w-100 p-4 border-2 border-dashed border-secondary rounded-4">
                                <i class="fas fa-cloud-upload-alt fa-4x text-primary mb-3"></i>
                                <h4>Drop Image to Generate Links</h4>
                            </label>
                            <input type="file" id="file" name="file" style="display:none;" onchange="document.getElementById('upForm').submit();">
                        </form>
                    </div>
                </div>
            </div>
            <div class="ad-slot">{{ s.ad_top | safe }}</div>
            <h4 class="mb-4 fw-bold">Recent Public Feed</h4>
            <div class="row g-4">
                {% for f in files %}
                <div class="col-md-3 col-6">
                    <div class="glass p-2">
                        <img src="/f/{{ f.file_id }}" class="poster-img mb-2">
                        <a href="/view/{{ f._id }}" class="btn btn-grad w-100 btn-sm">Get 16 Links</a>
                    </div>
                </div>
                {% endfor %}
            </div>
    ...
    <body>
        <div class="container py-4 text-center">
            ...
            <div class="glass p-4 mx-auto" style="max-width: 850px;">
                <h3 class="mb-4">Generated Links</h3>
                <img src="/f/{{ f.file_id }}" class="rounded shadow border border-primary mb-4" style="max-height: 300px;">
...
def login():
    ...
    <body style="background:#0f172a; display:flex; align-items:center; justify-content:center; height:100vh;">
        <div class="glass p-5 text-center" style="width:380px;">
            <h2 style="color:white;" class="mb-4">Admin Hub</h2>
            <form method="POST">
...
def admin_dashboard():
    ...
    <body>
        <nav class="nav-bar p-3 d-flex justify-content-between container-fluid"><span class="fw-bold">ADMIN PRO PANEL</span><div><a href="/admin" class="text-white me-3">Settings</a><a href="/admin/gallery" class="text-white me-3">Posters</a><a href="/logout" class="text-danger">Logout</a></div></nav>
        <div class="container mt-4">
            <div class="row g-4 mb-4 text-center">
                <div class="col-md-6"><div class="glass p-4"><h5>Total Posters</h5><h2 class="text-primary fw-bold">{{ poster_c }}</h2></div></div>
                <div class="col-md-6"><div class="glass p-4"><h5>Connected Bots</h5><h2 class="text-primary fw-bold">{{ bot_c }}</h2></div></div>
            </div>
            <form action="/save_settings" method="POST" class="glass p-5 row g-3">
                ...
                <div class="col-md-6"><label>Popunder Ad</label><textarea name="ad_popunder" class="form-control" rows="1">{{ s.ad_popunder }}</textarea></div>
                <div class="col-md-4"><label>Social Bar</label><textarea name="ad_social" class="form-control" rows="1">{{ s.ad_social }}</textarea></div>
                <div class="col-md-4"><label>Top Ad</label><textarea name="ad_top" class="form-control" rows="1">{{ s.ad_top }}</textarea></div>
                <div class="col-md-4"><label>Footer Ad</label><textarea name="ad_footer" class="form-control" rows="1">{{ s.ad_footer }}</textarea></div>
                <button class="btn btn-grad mt-4">Save Configuration</button>
            </form>
            <div class="row g-4 mt-2">
                <div class="col-md-6"><div class="glass p-4"><h6>Manage Bots</h6><form action="/add_bot" method="POST" class="input-group mb-2"><input type="text" name="token" class="form-control" placeholder="Token"><button class="btn btn-primary">Add</button></form>
                ...
                <div class="col-md-6"><div class="glass p-4"><h6>Manage Channels</h6><form action="/add_channel" method="POST" class="input-group mb-2"><input type="text" name="cid" class="form-control" placeholder="-100..."><button class="btn btn-dark">Add</button></form>
...
def admin_gallery():
    ...
    <body>
        <nav class="nav-bar p-3 d-flex justify-content-between container-fluid"><span class="fw-bold">GALLERY ({{ total }})</span><a href="/admin" class="text-white">Back Settings</a></nav>
        <div class="container mt-4"><div class="row g-4">
            ...
            <div class="d-flex gap-1"><a href="/view/{{ f._id }}" class="btn btn-primary btn-sm flex-grow-1">View</a><a href="/delete/{{ f._id }}" class="btn btn-danger btn-sm">Delete</a></div></div></div>
            {% endfor %}
        </div>
        <div class="d-flex justify-content-center my-5">{% if page > 1 %}<a href="?page={{ page-1 }}" class="btn btn-secondary me-2">Prev</a>{% endif %}<span class="p-2">Page {{ page }}</span>{% if page < total_p %}<a href="?page={{ page+1 }}" class="btn btn-secondary ms-2">Next</a>{% endif %}</div></div>
    </body></html>
    """, style=UI_STYLE, files=files, page=page, total_p=total_p, total=total)

# --- Webhook & Bot Logic (Custom API System) ---
    ...
    try:
        ...
        if update.message:
            ...
            if not check_join(bot, user.id):
                ...
                bot.send_message(msg.chat.id, "🛑 আপনি জয়েন নেই! জয়েন করুন।", reply_markup=markup)
                return "OK", 200
            ...
            if msg.text == "/start":
                ...
                stored_id = fs.put(f_content, filename="tg_poster", content_type="image/jpeg")
                ...
                bot.reply_to(msg, f"✅ Done! Download Link: {request.host_url}view/{str(ins.inserted_id)}")
    except: pass
    return "OK", 200

# --- System Standard Logic ---
